...
- [ ] **Memory Pipeline 优化** — 当前管线每轮增加延迟。探索批量/异步执行、自适应去重阈值、事实置信度评分。
- [ ] **文档库手动管理** — 在 Document Library 的 Manual 标签页支持手动创建/编辑条目。后端：添加 POST/PUT 端点。前端：添加新建表单 + 条目编辑功能。
- [ ] **飞书集成** — 双向同步：将飞书文档导入知识库，或将研究成果导出到飞书文档。
