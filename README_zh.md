...
- **文档摄入** — 支持 URL、PDF、纯文本导入，自动分块与向量化。支持两种分块策略：固定大小分块（段落优先，800 字符）和**语义分块**（基于嵌入的边界检测，通过相邻句子的余弦相似度识别语义断裂点）
...
ingest_document 节点
  ...
  ↓
分块（可配置策略）
  ├── "fixed"    → chunk_text(): 段落 → 句子 → 字符分割（800字符，100重叠）
  └── "semantic" → chunk_text_semantic(): 拆句 → 嵌入 → 余弦相似度 → 低相似度处断句
                   ├── 合并小于 min_chunk_size（200字符）的片段
                   ├── 拆分大于 max_chunk_size（1500字符）的片段
                   └── 相邻 chunk 间添加重叠
  ↓
...
**语义分块**通过计算相邻句子嵌入的余弦相似度来检测主题转换。当相似度低于阈值（默认 0.5）时，插入分块边界。这使得分块在语义上连贯，而非按固定大小切割。通过环境变量配置：`CHUNK_STRATEGY=semantic`、`CHUNK_SIMILARITY_THRESHOLD=0.5`、`CHUNK_MIN_SIZE=200`、`CHUNK_MAX_SIZE=1500`。
...
- [x] **文档库显式输出** — 结构化知识库可视化，支持文档集合浏览，类似飞书文档库的展示形式。将归档知识组织为可浏览、可分类的集合，支持元数据、标签与搜索。
- [ ] **飞书集成** — 与飞书进行双向同步：将飞书文档导入知识库，或将 Agent 研究成果导出到飞书文档。
- [x] **Cross-encoder 重排序** — 在混合检索后增加重排序阶段（如 `BAAI/bge-reranker-v2-m3`），优化结果排序。
- [x] **Recall Memory 集成** — 利用 recall_memory 中的对话历史，提供跨会话的长期对话上下文。
- [x] **文本分块优化** — 新增基于嵌入的语义分块（`chunk_text_semantic`），作为固定大小分块的替代方案。通过计算相邻句子嵌入的余弦相似度，在语义断裂点（低相似度处）切分。支持配置相似度阈值、最小/最大块大小和重叠。通过环境变量 `CHUNK_STRATEGY=semantic` 或 `strategy` 参数选择。
- [ ] **Memory Pipeline 优化** — 当前 mem0 风格管线（提取 → 去重 → 更新）在每次回复后运行，增加响应延迟。事实提取质量高度依赖 LLM 能力；去重使用固定余弦阈值（0.8），可能遗漏近似重复或过度合并不同事实。探索方向：批量/异步管线执行、自适应去重阈值、事实置信度评分、以及超越简单合并/跳过的冲突解决策略。
- [x] **自动整理文档重复内容** — 构建一个 Agent 定期扫描归档记忆中跨文档的重复或重叠内容（包括导入文档、研究摘要、对话事实）。使用嵌入聚类 + LLM 判断检测冗余，合并重叠条目，无需人工干预即可维护干净、有序的知识库。
