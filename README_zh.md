...
- [ ] **文档库显式输出** — 结构化知识库可视化，支持文档集合浏览，类似飞书文档库的展示形式。将归档知识组织为可浏览、可分类的集合，支持元数据、标签与搜索。
- [ ] **飞书集成** — 与飞书进行双向同步：将飞书文档导入知识库，或将 Agent 研究成果导出到飞书文档。
- [ ] **Cross-encoder 重排序** — 在混合检索后增加重排序阶段（如 `BAAI/bge-reranker-v2-m3`），优化结果排序。
- [ ] **Recall Memory 集成** — 利用 recall_memory 中的对话历史，提供跨会话的长期对话上下文。
- [ ] **文本分块优化** — 当前分块采用段落优先分割 + 固定块大小（800字符）+ 字符级重叠。存在问题：缺乏语义边界感知，对表格/列表/代码块处理不佳，固定块大小忽略内容结构。探索语义分块（基于嵌入的边界检测）或智能分块（LLM 引导分割）以提升检索质量。
...
- [ ] **自动整理文档重复内容** — 构建一个 Agent 定期扫描归档记忆中跨文档的重复或重叠内容（包括导入文档、研究摘要、对话事实）。使用嵌入聚类 + LLM 判断检测冗余，合并重叠条目，无需人工干预即可维护干净、有序的知识库。
